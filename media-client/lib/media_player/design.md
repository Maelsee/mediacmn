...
### 9.11 轨道名称统一格式化 (2026-01-21)

**目标**：解决音轨和字幕轨道名称显示杂乱（如直接显示 ID "1", "2"）的问题，统一全端显示规范。

**方案设计**：
1.  **统一格式化工具**：创建 `TrackFormatter` 工具类，集中处理 `media_kit` 轨道对象的命名逻辑。
2.  **命名规则**：
    *   **优先显示标题**：若有 `title`，优先显示。若 `title` 未包含语言信息且存在 `language`，则追加 `(语言)`。
    *   **语言代码映射**：若无 `title` 但有 `language`，将 ISO 639-1/2 代码映射为中文名称（如 `chi` -> `中文`）。
    *   **智能回退**：若无标题和语言，尝试解析数字 ID 并显示为 `音轨 N` / `字幕 N`（索引+1），避免显示原始 ID。
3.  **应用范围**：
    *   `AudioPanel`: 音轨选择列表。
    *   `SubtitlePanel`: 字幕选择列表。

**验证**：
*   音轨/字幕列表不再显示无意义的数字 ID。
*   语言代码正确转换为中文名称。
*   格式化逻辑覆盖了标题、语言、ID 的各种组合情况。