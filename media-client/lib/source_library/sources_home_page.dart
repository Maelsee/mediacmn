      ...
      child: ListTile(
        ...
        trailing: PopupMenuButton<String>(
          itemBuilder: (ctx) => const [
            PopupMenuItem(value: 'scan', child: Text('手动扫描')),
            PopupMenuItem(value: 'edit', child: Text('编辑')),
            PopupMenuItem(value: 'disable', child: Text('停用/启用')),
          ...
          onSelected: (v) async {
            ...
            switch (v) {
              case 'scan':
                ...
                try {
                  final taskId = await api.createScanTask(storageId: item.id);
                  ref
                      .read(tasksProvider.notifier)
                      .showSingleSourceTask(sourceId: item.id, taskId: taskId);
                  ref.read(tasksProvider.notifier).showTray();
                } catch (e) {
                  messenger
                      .showSnackBar(SnackBar(content: Text('扫描任务创建失败：$e')));
                }
                break;
              case 'edit':
                if (item.type == 'webdav') {
                  ...
                  final Map<String, String> params = {
                    ...
                    if ((detail['root_path'] as String?)?.isNotEmpty == true)
                  ...
                  final changed = await router.push('/sources/${item.id}/edit');
                  if (changed == true) {
                    await ref.read(sourcesProvider.notifier).load();
                  }
                }
                break;
              case 'disable':
                final enabled = item.status != 'enabled';
                await ref
                    .read(apiClientProvider)
                    .toggleSource(item.id, enabled: enabled);
                await ref.read(sourcesProvider.notifier).load();
                break;
              case 'delete':
              ...
              case 'test':
                final ok = await ref
                    .read(apiClientProvider)
                    .testStorageConnection(item.id);
                ref
                    .read(sourcesProvider.notifier)
                    .setStatus(item.id, ok ? 'connected' : 'disconnected');
