...
class MediaService:
    ...
    async def get_file_subtitles(
        ...
        # 准备 WebDAV 基础信息，用于拼装字幕 URL（仅对 webdav 类型生效）
        source_type = None
        base_url = None
        ...
        for entry in entries:
            ...
            size_text = self._to_human_size(entry.size) if entry.size else None
            language = None
            try:
                parts = name.rsplit(".", 2)
                if len(parts) == 3:
                ...
                asset_items.append(
                    {
                        ...
                        "path": fa.full_path,
                        "size": fa.size,
                        "size_text": self._to_human_size(fa.size),
