...
class SidecarLocalizeProcessor:
        ...
        try:
            ...
            with next(get_db_session()) as session:
                ...
                if not storage_client:
                    logger.error(f"侧车本地化失败，无法获取存储客户端: storage_id={storage_id}")
                    return False
                ...
                media_type = self._determine_media_type(session, core, media_file)

                provider = core.canonical_source
                external_key = core.canonical_external_key
                if not provider or not external_key:
                    logger.warning("缺少规范外部ID，尝试回退：查找任一外部ID")
                    ext = session.exec(select(ExternalID).filter(ExternalID.user_id == media_file.user_id, ExternalID.core_id == core.id)).first()
                    if not ext:
                ...
                try:
                    if media_type == MediaType.MOVIE and hasattr(plugin, "get_movie_details"):
                        ...
                        ep = session.exec(select(EpisodeExt).filter(EpisodeExt.user_id == media_file.user_id, EpisodeExt.core_id == core.id)).first()
                        series_id = None
                        series_provider = None
                        season_no = None
                        episode_no = None
                        if ep and ep.series_core_id:
                            sc = session.exec(select(MediaCore).filter(MediaCore.id == ep.series_core_id)).first()
                            if sc:
                                series_provider = getattr(sc, "canonical_source", None)
                                series_id = getattr(sc, "canonical_external_key", None)
                                if not series_provider or not series_id:
                                    ext2 = session.exec(select(ExternalID).filter(ExternalID.user_id == media_file.user_id, ExternalID.core_id == sc.id)).first()
                                    if ext2:
                                        series_provider = series_provider or getattr(ext2, "source", None)
                                        series_id = series_id or getattr(ext2, "key", None)
                ...
                if nfo_valid and (poster_valid or fanart_valid):
                    try:
                        with next(get_db_session()) as s2:
                            ...
                            if core2:
                                try:
                                    ...
                                    if core2.kind == 'movie':
                                        ...
                                        se2 = s2.exec(select(SeasonExt).where(SeasonExt.user_id == media_file.user_id, SeasonExt.core_id == core2.id)).first()
                                        if se2:
                                            se2.nfo_path = nfo_path
                                except Exception:
                                    pass
                                from models.media_models import Artwork
                                poster_row = s2.exec(select(Artwork).where(Artwork.user_id == media_file.user_id, Artwork.core_id == core2.id, func.lower(Artwork.type) == "poster")).first()
                                backdrop_row = s2.exec(select(Artwork).where(Artwork.user_id == media_file.user_id, Artwork.core_id == core2.id, func.lower(Artwork.type) == "backdrop")).first()
                                if not backdrop_row:
                                    backdrop_row = s2.exec(select(Artwork).where(Artwork.user_id == media_file.user_id, Artwork.core_id == core2.id, func.lower(Artwork.type) == "fanart")).first()
                                if poster_valid:
                                    if poster_row:
    ...
    def _determine_media_type(self, session, core: MediaCore, media_file: FileAsset) -> MediaType:
        """
        ...
        """
        try:
            ep = session.exec(select(EpisodeExt).filter(EpisodeExt.core_id == media_file.core_id, EpisodeExt.user_id == media_file.user_id)).first()
            if ep:
                return MediaType.TV_EPISODE
            if core.kind in ("tv_series", "tv_season"):
