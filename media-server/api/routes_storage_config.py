...
# 核心依赖
from core.db import get_async_session  # ✅ 切换为异步 Session 获取函数
...
async def get_storage_statistics(
    ...
    # 调用优化后的异步统计方法
    return await storage_service.get_storage_statistics(db, user_id)
...
async def list_storages(
    ...
    user_id = get_user_id(current_subject)
    # 调用异步服务层
    return await storage_service.list_user_storages(db, user_id, storage_type)


@router.get("/{storage_id}", response_model=Dict[str, Any])
async def get_storage(
    ...
    storage_config = await storage_service.get_storage_config(db, storage_id, user_id)
    if not storage_config:
        raise HTTPException(status_code=404, detail="Storage configuration not found")
...
async def create_storage(
    ...
    try:
        # 使用优化后的事务创建方法
        storage_config = await storage_service.create_storage_config(db, user_id, request)
        ...
        logger.error(f"创建存储配置失败: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error during creation")


@router.put("/{storage_id}", response_model=Dict[str, Any])
...
async def delete_storage(
    ...
    success = await storage_service.delete_storage_config(db, storage_id, user_id)
    
    if not success:
        raise HTTPException(status_code=404, detail="Storage configuration not found")
    
    return {"message": "Storage configuration deleted successfully"}
