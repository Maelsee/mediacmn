...
- 扩展：电影 `MovieExt`、剧集 `SeriesExt/SeasonExt/EpisodeExt`；`EpisodeExt` 已包含 `episode_type` 并注释了 `absolute_episode_number`（`models/media_models.py:205, 218`）。
...
- `SeriesExt` 增加可选 `category_hint`，用于聚合与筛选；如不改表，可用 `group_key/canonical_source` 进行归类（`models/media_models.py:58-62`）。
