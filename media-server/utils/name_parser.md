...
- 目录结构中，父目录为 `唐朝诡事录`，子目录为 `唐朝诡事录之西行S02`，当前逻辑在清洗季信息时对 `S02` 做了替换，但没有对“之西行”部分做额外保护，后续又在 title 选择时被更下层的 `4K` 干扰。
...
2. 在 `_preprocess_name()` 中：
   - 对仅含 `4K`、`1080p` 等技术片段的文件名，保持现有标题由父目录提供，不允许技术分段覆盖从路径提取的中文标题。
